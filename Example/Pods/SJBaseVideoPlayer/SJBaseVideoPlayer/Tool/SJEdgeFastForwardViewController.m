...
@interface _SJShapeLayer : CAShapeLayer<CAAnimationDelegate>
@property (nonatomic, copy, nullable) void(^animationDidStopExeBlock)(_SJShapeLayer *layer);
@end

@implementation _SJShapeLayer
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if ( _animationDidStopExeBlock ) _animationDidStopExeBlock(self);
}
@end
...
- (instancetype)initWithFrame:(CGRect)frame position:(SJFastForwardTriggeredPosition)position {
    ...
    {
        NSMutableArray<_SJShapeLayer *> *m = [NSMutableArray new];
        CGFloat size = 8;
        for ( int i = 0 ; i < 3 ; ++ i ) {
...
- (void)showAnimations {
    [self _addOpacityAnimationToLayer:_roundLayer delegate:nil duration:1];
    [self _recursiveAnimation:self.triangles.count - 1];
    [self _addOpacityAnimationToLayer:_textLayer delegate:nil duration:1];
}
...
- (void)_recursiveAnimation:(NSInteger)idx {
    ...
    _SJShapeLayer *layer = self.triangles[idx];
    [self _addOpacityAnimationToLayer:layer delegate:layer duration:0.3];
    __weak typeof(self) _self = self;
    layer.animationDidStopExeBlock = ^(_SJShapeLayer *layer) {
...
- (void)_addOpacityAnimationToLayer:(CALayer *)layer delegate:(nullable id<CAAnimationDelegate>)delegate duration:(NSTimeInterval)duration {
    CABasicAnimation *anim = [CABasicAnimation animationWithKeyPath:@"opacity"];
    anim.fromValue = @(1);
    anim.toValue = @(0.001);
    anim.duration = duration;
    anim.delegate = delegate;
    [layer addAnimation:anim forKey:@"triangle.opacity"];
}
@end
