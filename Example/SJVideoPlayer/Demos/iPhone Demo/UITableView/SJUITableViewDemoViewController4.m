...
@interface SJUITableViewSectionHeaderView : SJBaseTableViewHeaderFooterView
@property (nonatomic, strong) UIImageView *avatarImageView;
@property (nonatomic, strong) UILabel *usernameLabel;
@property (nonatomic, strong) UIView *playerSuperview;
@property (nonatomic, strong) UIImageView *playImageView;

@property (nonatomic, copy, nullable) void(^playerSuperviewWasTapped)(SJUITableViewSectionHeaderView *headerView);
@end

@implementation SJUITableViewSectionHeaderView
...
@interface SJUITableViewDemoViewController4 ()

@end

@implementation SJUITableViewDemoViewController4
- (void)viewDidLoad {
    [super viewDidLoad];
    
    [SJUITableViewSectionHeaderView registerWithTableView:self.tableView];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    SJUITableViewSectionHeaderView *view = [SJUITableViewSectionHeaderView reusableViewWithTableView:tableView];
    __weak typeof(self) _self = self;
    view.playerSuperviewWasTapped = ^(SJUITableViewSectionHeaderView *headerView) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
        
        if ( !self.player ) self.player = SJVideoPlayer.player;
        
        self.player.URLAsset = [SJVideoPlayerURLAsset.alloc initWithURL:SourceURL0 playModel:[SJPlayModel playModelWithTableView:self.tableView inHeaderForSection:section superviewSelector:NSSelectorFromString(@"playerSuperview")]];
    };
    return view;
}
