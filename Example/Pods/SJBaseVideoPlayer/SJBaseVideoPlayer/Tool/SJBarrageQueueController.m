...
@property (nonatomic, copy, nullable) void(^animationDidStopExeBlock)(SJBarrageView *view, BOOL isFinished);
@end
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if ( _animationDidStopExeBlock != nil ) _animationDidStopExeBlock(self, flag);
}
@end
...
@implementation CALayer (SJAnimationControlAdd)
- (void)pauseAnimation {
    ...
    for ( NSInteger i = 0 ; i < _containerViews.count ; ++ i ) {
        ...
        if ( time >= last.nextBarrageStartTime + last.delay ) {
            ...
            if ( item != nil ) {
                ...
                animation.delegate = barrageView;
                animation.fillMode = kCAFillModeForwards;
                animation.removedOnCompletion = NO;
                [barrageView.layer addAnimation:animation forKey:@"anim"];
                __weak typeof(self) _self = self;
                barrageView.animationDidStopExeBlock = ^(SJBarrageView * _Nonnull view, BOOL isFinished) {
                    __strong typeof(_self) self = _self;
                    if ( !self ) return;
                    [view removeFromSuperview];
                    [self.reusablePool addBarrageView:view];
                    [self _pauseClockIfNeeded];
                };
            }
        }
