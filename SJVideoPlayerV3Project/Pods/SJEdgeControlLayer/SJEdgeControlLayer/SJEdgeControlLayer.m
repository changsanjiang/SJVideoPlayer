    ...
    return ^(void(^block)(SJVideoPlayerSettings *settings)) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ( !self ) return ;
                [[NSNotificationCenter defaultCenter] postNotificationName:SJSettingsPlayerNotification object:[SJVideoPlayerSettings commonSettings]];
            });
        });
    };
}
...
- (void)rightControlView:(SJVideoPlayerRightControlView *)view clickedBtnTag:(SJVideoPlayerRightViewTag)tag {
    if ( tag == SJVideoPlayerRightViewTag_FilmEditing ) {
        if ( [self.delegate respondsToSelector:@selector(clickedRightBtnOnControlLayer:)] ) {
            [self.delegate clickedRightBtnOnControlLayer:self];
        }
    }
}
...
- (void)setEnableFilmEditing:(BOOL)enableFilmEditing {
    ...
    if ( enableFilmEditing ) {
        ...
        _rightControlView.disappearTransform = CGAffineTransformMakeTranslation(_rightControlView.intrinsicContentSize.width, 0);
        _rightControlView.filmEditingBtnImage = self.settings.filmEditingBtnImage;
        if ( !self.videoPlayer.isFullScreen ) [_rightControlView disappear];
        else if ( !self.videoPlayer.controlLayerAppeared ) [_rightControlView disappear];
    }
    else {
...
- (void)_controlViewLoadSetting {
    // load setting
    SJEdgeControlLayer.update(^(SJVideoPlayerSettings * _Nonnull commonSettings) {});
